...
if os.path.isfile('flip_config.npy'):
    start_array = np.load('flip_config.npy')
    start_epoch = start_array[0]
    start_iter = start_array[1]
    not_already_configured_epoch = True
    not_already_configured_iter  = True
    net.load_state_dict(torch.load("model_trained/flip_trained_model.ptch"))
else:
...
pic_dataset = objDataset(descr_file = data_descr_path+'/train_annotations.txt')
...
# TRAINING
    ...
    for i in range(len(pic_dataset)):
        ...
        image, obj, ground, image_flip1, obj_flip1, ground_flip1, image_flip2, obj_flip2, ground_flip2, image_flip12, obj_flip12, ground_flip12 = pic_dataset[i]
        ...
        if i % 500 == 499:
            ...
            print()
            running_loss = 0.0
            torch.save(net.state_dict(), "model_trained/flip_trained_model.ptch")
            np.save("flip_config", np.array([epoch, i]))


    print("finished", epoch)
    torch.save(net.state_dict(), "model_trained/flip_alpha_trained_model_after_"+str(epoch)+"epoch"+".ptch")
