...
class sumNet3(nn.Module):
    def __init__(self):
        super(sumNet3, self).__init__()

        self.convNet = convNet()
        self.convNet.load_state_dict(torch.load("data_descript/3-salface_path.ptch"))
        ...
        self.resnet1.load_state_dict(torch.load("data_descript/4-face_path.ptch"))
        self.resnet2 = resnet18(pretrained=False)
        self.resnet2.fc = nn.Sequential(nn.Conv2d(512, 64, kernel_size=5, padding=2),
                                        nn.Conv2d(64, 32, kernel_size=5, padding=2),
                                        nn.Conv2d(32, 1, kernel_size=5, padding=2))
        self.resnet2.load_state_dict(torch.load("data_descript/1-obj_path.ptch"))

    def forward(self, full, face, obj):
        # --part full
        x = full
        ...
        x = obj
        x = self.resnet2(x)
        x = F.interpolate(x, size=(obj.shape[2], obj.shape[3]), mode='bilinear', align_corners=False)
        out_obj = x

        # --part final
        final_out1 = out_data * out_obj
        final_out1 = F.sigmoid(final_out1)
        final_out2 = out_data * out_eyes
