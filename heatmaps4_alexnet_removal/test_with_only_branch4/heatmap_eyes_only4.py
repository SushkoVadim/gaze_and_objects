...
import time
import sys
from convNet_only4 import sumNet_only4
from eyes_dataset_flip_only4 import eyesDataset
...
if os.path.isfile('flip_config_eyes_only4.npy'):
    start_array = np.load('flip_config_eyes_only4.npy')
    start_epoch = start_array[0]
    start_iter = start_array[1]
    not_already_configured_epoch = True
    not_already_configured_iter  = True
    net.load_state_dict(torch.load("model_trained/flip_eyes_trained_model_only4.ptch"))
else:
...
# TRAINING
    ...
    for i in range(len(eyes_dataset)):
        ...
        if i % 500 == 499:
            ...
            print()
            running_loss = 0.0
            torch.save(net.state_dict(), "model_trained/flip_eyes_trained_model_only4.ptch")
            np.save("flip_config_eyes_only4", np.array([epoch, i]))


    print("finished", epoch)
    torch.save(net.state_dict(), "model_trained/flip_eyes_trained_model_only4_after_"+str(epoch)+"epoch"+".ptch")
