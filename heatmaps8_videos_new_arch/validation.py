...
def validate(trainloader_test, net, device, epoch):
    ...
    with torch.no_grad():
        ...
        for i, obj in enumerate(trainloader_test):
            ...
            #plt.imshow(source_image[0, :, :, :].view(400, 400, 3).detach().cpu().numpy().astype(int))
            #plt.show()
            #plt.imshow(target_image[0, :, :, :].view(400, 400, 3).detach().cpu().numpy().astype(int))
            #plt.show()
            #plt.imshow(output[0, 0, :, :].detach().cpu().numpy(), cmap='hot')
            #plt.show()
            print(sigmoid)
...
if os.path.isfile('video_config.npy'):
    start_array = np.load('video_config.npy')
    start_epoch = start_array[0]
    start_iter = start_array[1]
    not_already_configured_epoch = True
    not_already_configured_iter  = True
    net.load_state_dict(torch.load("model_trained/video_trained_model.ptch"))
else:
